...
#  
    ...
    # $cycle其实就是0-11的数字
    # tables中的每个table本质上也是一个数组，在相应的位置（比如TABLE_ROW位置）上放置这相应的属性
...
#	$field_copy[FIELD_INDEX] = 'nokey' if $field_copy[FIELD_INDEX] eq '';
        ...
        # 字段索引信息 $field_copy[FIELD_INDEX]就是用户配置的索引信息一般为key或""
        if (
    ...
    # 创建数据库 这是一个大循环,对每个数据库都会创建相同的表和数据
    ...
    # 开始拼接并执行具体的sql
        ...
        foreach my $table_id (0..$#tables) {
            ...
            # 存储meta info
            foreach my $f (@fields_copy){
                my $index_type = ($f->[FIELD_NAME] eq 'pk'? 'primary': (defined $f->[FIELD_INDEX_SQL]? 'indexed': 'ordinary'));
                my @meta_row = ($schema, $table->[TABLE_NAME], 'table', $f->[FIELD_NAME], $index_type);
                push(@$meta_info, \@meta_row);
            }
            ...
            if (not ($executor->type() == DB_MYSQL || 
                ...
                foreach my $idx (@index_fields) {
                    ...
                    if ($key_sql =~ m/^key \((`[a-z0-9_]*)/) {
                        my $fname = substr($1, 1);
                        my $identifier = substr($fname.'_'.$table->[TABLE_NAME], 0, 60);
                        $executor->execute("CREATE INDEX idx_".
                                        $identifier.
                                        " ON ".$table->[TABLE_NAME]."($fname)");
                    }
                }
            }
    ...
    # for debug
    # print "meta info:\n";
    # for my $col (@$meta_info){
    #     print "@$col\n";
    # }

    GenTest::set_cached_meta($meta_info);
